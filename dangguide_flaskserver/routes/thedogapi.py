...
def fetch_breeds(limit: int) -> List[Dict[str, Any]]:
    ...
    resp = s.get(f"{DOG_API_BASE_URL}/breeds", params=params, timeout=HTTP_TIMEOUT)

    if resp.status_code == 401:
        raise RuntimeError(("UNAUTHORIZED", 502, "TheDogAPI 키 오류 또는 미설정"))
    if resp.status_code >= 500:
        raise RuntimeError(("UPSTREAM_ERROR", 502, "TheDogAPI 서버 오류", {"status": resp.status_code}))
    if resp.status_code != 200:
        raise RuntimeError(("UPSTREAM_BAD_RESPONSE", 502, "TheDogAPI 비정상 응답", {"status": resp.status_code}))

    try:
        return resp.json()
    except Exception as e:
        raise RuntimeError(("UPSTREAM_PARSE_FAILED", 502, f"TheDogAPI 응답 파싱 실패: {e}"))
...
def get_dog_breeds():
    ...
    try:
        ...
        if limit <= 0 or limit > 50:
            return _err("limit는 1~50 사이여야 합니다.", 400, error_code="INVALID_PARAM")
    except ValueError:
        return _err("limit는 정수여야 합니다.", 400, error_code="INVALID_PARAM")

    translate_flag = (request.args.get("translate", "false").lower() in ("true", "1", "yes"))

    if not DOG_API_KEY:
        return _err("DOG_API_KEY 환경변수가 설정되지 않았습니다.", 500, error_code="NO_API_KEY")
    ...
    try:
        ...
        return _err("알 수 없는 서버 오류", 500, error_code="UNKNOWN_ERROR", details={"reason": str(e)})
